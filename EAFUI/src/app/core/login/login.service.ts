...
export class LoginService {
  private loginServiceUrl = environment.serviceUrl + 'AuthenticationService.svc'
   ...
   */
  login(username, password): Observable<LoginToken> {
  ...
  validateOAuthToken(accessToken: string, id: string) {
    return this.http.post<LoginToken>(`${environment.serviceUrl}SingleSignOnService.svc/ValidateToken`, {
  ...
  parseToken(token) {
    ...
    this.authorizationService.getLoggedInUserRoleData().subscribe(result => {
      ...
      if (landingPageType == LandingPageTypes.Default) {
        //this.DefaultDataService.SomeDefaultDataAccessMethod()
      }
      const userLandingPageUrl = UserLandingPages.landingPages.get(landingPageType).url
      this.router.navigate([`${userLandingPageUrl}${url}`])
    })
  }
  ...
  logout() {
      ...
      if (this.localStorageService.loginMethodCookie === 'Local') {
        this.router.navigate(['/localcredentials'])
      } else {
        this.router.navigate(['/'])
      }
      this.localStorageService.loginMethodCookie = null
  }
