  ...
  displaySnackbar(errorResponse: ErrorResponse | Blob) {
    ...
    if (errorResponse instanceof Blob) {
      ...
      fr.onload = () => {
        ...
        this.snackBar.open(result.Message, null, {duration: environment.snackBarDuration})
      }

      fr.readAsText(errorResponse)
    } else {
      this.snackBar.open(errorResponse.Message, null, {duration: environment.snackBarDuration})
    }
  }
