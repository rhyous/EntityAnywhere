...
export class EntityService {
  ...
  pageEvent: PageEvent
  private pluralizePipe = new PluralizePipe()
  ...
  getAllData(entityName: string): Observable<any> {
    const entityPath = entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName)
    return this.http.get<Response>(environment.serviceUrl + entityPath, this.getHeader())
  }

  getEntityList(entityName: string, top: number = null, skip: number = null): Observable<any> {
    let requestUri = environment.serviceUrl + entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName)
  ...
  getExpandedEntityList(entityName: string, expandedEntities: string[], top: number = null, skip: number = null)
  ...
  getEntityFilteredList(entityName: string, filterText: string, properties: string[], orderBy: string = null, top: number = null,
    skip: number = null, nonContainsProperties: string[] = [])
    : Observable<any> {
  ...
  getFilteredEntityList<T>(entityName: string, filter: string, top: number = null, skip: number = null): Observable<ODataCollection<T>> {
    let requestUri = environment.serviceUrl + entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName)
  ...
  getEntityListByCustomUrl<T>(entityName: string, customUrl: string): Observable<T> {
    const requestUri = environment.serviceUrl + entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName) + '/' + customUrl
  ...
  getFilteredMapList(entityName: string, targetEntity: string, mappedEntity: string, mappedFilter: string, entityId: number,
    ...
    const filter = mappedFilter !== undefined ? `($filter=${mappedFilter})` : ''
    let requestUri = environment.serviceUrl + entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName) + `(${entityId})`
  ...
  getEntityMetaData(entityName: string)  {
    return this.http.get<EntitySchema>(environment.serviceUrl + entityName + 'Service.svc/$metadata', this.getHeader())
  }

  getEntityData(entityName: string, entityId: any): Observable<any> {
    const entityPath = entityName + 'Service.svc/' + this.pluralizePipe.transform(entityName) + `(${entityId})`
    return this.http.get<Response>(environment.serviceUrl + entityPath, this.getHeader())
  }

  addEntity(entityName: string, entities: any[]): Observable<ODataCollection<any>> {
    const servicePath = `${entityName}Service.svc/${this.pluralizePipe.transform(entityName)}`
  ...
  putEntity(entityName: string, entityId: string, entityObject: any): Observable<any> {
    const servicePath = `${entityName}Service.svc/${this.pluralizePipe.transform(entityName)}(${entityId})`
  ...
  bulkDeleteEntities(entityName: string, entityIds: string[]): Observable<string> {
    const servicePath = `${entityName}Service.svc/${this.pluralizePipe.transform(entityName)}/Bulk`
