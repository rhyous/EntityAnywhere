...
import { MatSort, PageEvent,  MatDialog, MatSnackBar, MatTableDataSource } from '@angular/material'
...
export class EntityListComponent implements OnInit, AfterViewInit {
  ...
  singularizePipe = new SingularizePipe()

  count: number
  entityName: string
  entityNamePlural: string
  entityProperties: any[]
  nonContainsProperties = []
  ...
  initialise() {
    ...
    this.entityFilters.valueChanges.pipe(debounceTime(800)).subscribe(filter => this.filter(filter))
  ...
  rowClick(entityRow) {
    if (!window.getSelection().toString()) {
      this.router.navigate([`./admin/data-management/${this.entityName}/${entityRow.Id}`])
    }
  }
    ...
    if (response.Entities !== undefined) {
      response.Entities.forEach(entity => {
        if (entity.Object !== undefined) {
          this.metaDataObject.Fields.where(x => x.isAuditable() === false).select<string>(x => x.Name).forEach((prop) => {
            ...
            if (metadataField
              ...
              const metaObj = meta.firstOrDefault(x => x.key === metaField.NavigationKey)
  ...
  getEntityPropertiesFromMetadata() {
    ...
    const entity = md.find(x => x.key === this.entityName)
    if (entity === undefined) {
  ...
  processMetaData(schema: EntityMetadata): any[] {
    ...
    schema.Fields.forEach(field => {
      if (!field.isAuditable() && !field.isNavigationProperty() && !field.isMapping()) {
  ...
  createNew() {
    this.router.navigate([`./admin/data-management/${this.entityName}/add`])
  }
  ...
  multiUpdate() {
    this.matDialog.open(EntityMultiUpdateDialogComponent,
    {
      ...
      if (response === 'Update successful') {
        this.snackBar.open(this.entityNamePlural + ' have been updated successfully', null, {duration: environment.snackBarDuration})
        this.updateGridData(true)
      }
    })
  }
