...
export class CommonDialogBaseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subject.pipe(debounceTime(500)).subscribe(filterText => {this.onFilterChanged()})
  }
  ...
  setupFilterFields(md: EntityMetadata) {
    ...
    this.displayedColumns = this.filterFields.map(x => x.Name)
  }
  ...
  resolveFilterService(service) {
    service.subscribe(results => {
        let entities = []
  ...
  onKeyUp(event) {
    this.subject.next(this.form.get(event.target.attributes.placeholder.value).value)
  }

  getPropValue(row, column) {
