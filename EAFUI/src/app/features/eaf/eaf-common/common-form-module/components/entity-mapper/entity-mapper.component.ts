...
export class EntityMapperComponent implements OnInit {
  ...
  loadTable(): any {
    ...
    this.targetEntity = this.field.entityAlias === undefined ? this.getTarget(this.field.targetEntity, this.field.alias) : this.field.mapEntity
    ...
    const mapEntityFilter = this.field.entityAlias !== undefined ? `${this.field.entityAlias}Id eq ${this.field.value}` : undefined
  ...
  processMetadata() {
    ...
    const mColumns = this.mapEntityMetaData.Fields.filter(x => x.isVisibleOnRelationsMapper())
  ...
  formatData(data: any) {
    ...
    if (data.RelatedEntityCollection) {
      data.RelatedEntityCollection.find(x => x.RelatedEntity === firstRelatedEntity)
        .RelatedEntities.forEach(item => {
          ...
          if (this.field.entityAlias !== undefined) {
            ...
            if (item.RelatedEntityCollection) {
              item.RelatedEntityCollection.find(x => x.RelatedEntity === secondRelatedEntity)
              .RelatedEntities.forEach(mappedItem => {
  ...
  checkIfMappedDataNeeded(object): boolean {
  ...
  getCount(results) {
    if (results.RelatedEntityCollection) {
      if (results.RelatedEntityCollection.find(x => x.RelatedEntity === this.targetEntity)) {
        return results.RelatedEntityCollection.find(x => x.RelatedEntity === this.targetEntity).Count
      }
    }
    return 0
  }
  ...
  rowClick(row) {
    this.router.navigate([`admin/data-management/${this.field.targetEntity}/${row.Id}`])
  }
  ...
  processDeletes(entityIds: number[]) {
    ...
    forkJoin(getMapCalls).subscribe(results => {
      if (results.length > 1) {
        const ids = []
      ...
      this.snackBar.open(`There was an issue deleting the selected ${this.field.label}`,
                         null, { duration: environment.snackBarDuration })
    },                              () => {
      this.displayProgress = false
      this.snackBar.open(`The ${this.field.label} were successfully deleted`,
                         null, { duration: environment.snackBarDuration }).afterDismissed().subscribe(() => {
  ...
  getTarget(target: string, alias: string): string {
    return alias !== undefined ? alias : target
  }
