...
import { MatDialog, MatSnackBar, MatPaginator, MatTableDataSource, MatSort } from '@angular/material'
...
export class ExtensionEntityComponent implements OnInit {
    ...
    this.entityService.getExpandedFilteredEntityList(this.parentEntityName,
      ...
      .subscribe((success) => {
        ...
        if (success.Entities && success.Entities[0].RelatedEntityCollection) {
          const extensionEntities = success.Entities[0].RelatedEntityCollection[0].RelatedEntities
          extensionEntities.forEach(element => {
  ...
  rowClick(row) {
    ...
    this.router.navigate([`admin/data-management/${name}/${row.Id}`])
   }
    ...
    confirmationDialog.afterClosed().subscribe(result => {
      ...
      this.selection.selected.forEach(x => {
        ...
        .subscribe((response) => {
          ...
          this.snackBar.open(`The selected ${this.extensionEntityData.Name} were deleted`, null, { duration: 1000 })
        },
                   (error) => {
