...
export class EntitiesListComponent implements OnInit {
  ...
  toEntityList(entityName: string) {
    this.router.navigate([`/admin/data-administration/${this.pluralize.transform(entityName)}`])
  }

  ngOnInit() {

    this.entityService.getEntities().subscribe(entities => { // get all of the entities from the entity service
      const filtered = Object.keys(entities.EAF).filter(x => x.indexOf('$') === -1) // Remove the entities starting with $
      const grouped = filtered.select<EntityGrouping>(x => new EntityGrouping(x, entities.EAF[x]['@EAF.EntityGroup']))
      // create an EntityGrouping
      const filtered2 = grouped.filter(x => this.localStorageService.User?.hasRole('Admin')
                                         || this.userDataService.permittedEntitiesForUser.includes(x.entityName))
      this.entitiesArray = filtered2.filter(x => x.entityGroup !== undefined) // Remove the entities that haven't got a group
