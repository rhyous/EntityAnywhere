...
setupForm(ent: EntityMetadata) {
  const auditableProperties = this.wellKnownProperties.auditableProperties
  ent.Fields.filter(field => this.canDisplayField(auditableProperties, ent.Key, field))
    .forEach(field => {
       field.Required = false
       const fieldConfig = this.entityConfigBuilder.build(this.entityName, field, null,
...
getPatchEntityCollection(combinedObject: any) {
    ...
    const patchObjects: any = []
    this.selectedRecordIds.forEach((recordId: any) => {
