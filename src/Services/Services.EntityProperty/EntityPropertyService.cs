...
namespace Rhyous.EntityAnywhere.Services
{
          ...
          IEntityPropertyService
    {
        ...
        public EntityPropertyService(IServiceHandlerProvider<EntityProperty, IEntityProperty, int> serviceHandlerProvider,
        ...
        {
            var existing = _ServiceHandlerProvider.GetByIdHandler.Get(id);
            if (existing == null)
                return true;
            var entityId = existing.EntityId;
            var result = _ServiceHandlerProvider.DeleteHandler.Delete(id);
            if (!result)
                return result;
            var remaining = _ServiceHandlerProvider.QueryableHandler.GetQueryable(e => e.EntityId == entityId, -1, -1, nameof(EntityProperty.Order), SortOrder.Ascending).ToList();
            var updated = _PropertyOrderSorter.UpdateSortOrder(remaining);
            _ServiceHandlerProvider.UpdateHandler.Update(updated, nameof(EntityProperty.Order));
            return result;
        }
        ...
        /// </summary>
        /// <param name="entities">The EntityProperties to add.</param>
        /// <returns>The Added entity properties.</returns>
        public override async Task<List<IEntityProperty>> AddAsync(IEnumerable<IEntityProperty> entities)
        {
            var parentEntityIds = entities.Select(e => e.EntityId);
            var existingEntityProperties = _ServiceHandlerProvider.QueryableHandler.GetQueryable(e => parentEntityIds.Contains(e.EntityId));
            var existingMap = _PropertyOrderSorter.Sort(existingEntityProperties, e => e.EntityId, e => e.Order, e => e.Name);
            var addMap = _PropertyOrderSorter.Sort(entities, e => e.EntityId, e => e.Order, e => e.Name);
            ...
            foreach (var kvp in existingMap)
                updated.AddRange(_PropertyOrderSorter.UpdateSortOrder(kvp.Value));
            entities = entities.OrderBy(e => e.EntityId).ThenBy(e => e.Order);
            var result = await base.AddAsync(entities);
            updated.RemoveAny(entities);
            if (updated.Any())
                _ServiceHandlerProvider.UpdateHandler.Update(updated, nameof(EntityProperty.Order));
            return result;
        }        
        ...
        public override IEntityProperty Update(int id, PatchedEntity<IEntityProperty, int> patchedEntity)
        {
            ...
            var prior = _ServiceHandlerProvider.GetByIdHandler.Get(id);
            var existing = _ServiceHandlerProvider.QueryableHandler.GetQueryable(e => e.EntityId == prior.EntityId && e.Id != prior.Id, -1, -1, nameof(EntityProperty.Order))
                                                  .ToList();
            _PropertyOrderSorter.SafeInsert(existing, patchedEntity.Entity);
            var updated = _PropertyOrderSorter.UpdateSortOrder(existing).ToList();
