    ...
    public class QueryableHandlerTests
    {
        ...
        private QueryableHandler<TEntity, TInterface, TId> CreateQueryableHandler()
        {
            return new QueryableHandler<TEntity, TInterface, TId>(
                _MockRepo.Object,
                _MockEntityConfigurationProvider.Object,
        ...
        public void QueryableHandler_GetQueryable_ForwardsToRepo_Test()
        {
            ...
            {
                DefaultSortByProperty = nameof(TEntity.Name),
                DefaultSortOrder = SortOrder.Descending
            };
            _MockEntityConfigurationProvider.Setup(m => m.Provide(typeof(TEntity)))
                     .Returns(config);
            _MockRepo.Setup(m => m.Get(config.DefaultSortByProperty, config.DefaultSortOrder))
                     .Returns(queryable);
        ...
        public async Task QueryableHandler_Get_TwoQuotesToOneQuote_Json(TestDataRow<string> row)
        {
            ...
            _MockEntityConfigurationProvider.Setup(m => m.Provide(It.IsAny<Type>())).Returns(new EntityConfiguration());
