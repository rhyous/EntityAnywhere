...
namespace Rhyous.EntityAnywhere.Repositories.Common.Tests
{
    ...
    public class SqlRepositoryTests
    {
        ...
        private IEntityInfo<User> _EntityInfo;
        private Mock<IBaseDbContext<User>> _MockUserDbContext;
        private Mock<DbSet<User>> _MockUserDbSet;
        private Mock<IDynamicPropertySelector<User>> _MockDynamicPropertySelector;
        ...
        public void TestInitialize()
        {
            ...
            _EntityInfo = new EntityInfo<User>();
            _MockUserDbContext = _MockRepository.Create<IBaseDbContext<User>>();
            _MockUserDbSet = _MockRepository.Create<DbSet<User>>();
            _MockDynamicPropertySelector = _MockRepository.Create<IDynamicPropertySelector<User>>();
        }

        public SqlRepository<User, IUser, int> CreateBaseRepository()
        {
            return new SqlRepository<User, IUser, int>(
                _MockBaseDbContextCreator.Object,
                _EntityInfo,
                _MockDynamicPropertySelector.Object);
        }
        ...
        [TestMethod]
        [ListTNullOrEmpty(typeof(User))]
        public void SqlRepository_InsertSeedData_NullOrEmptyItems_Test(List<User> users)
        {
            // Arrange
            var baseRepository = CreateBaseRepository();

            // Act
            var result = baseRepository.InsertSeedData(users);
        ...
        [TestMethod]
        public void SqlRepository_InsertSeedData_ItemsValid_Test()
        {
            // Arrange
            var users = new List<User> { new User { Id = 1 }, new User { Id = 2 } };
            var dbSetUserList = new List<User>();
            ...
            _MockUserDbContext.Setup(x => x.SaveChanges()).Returns(1);
            _MockBaseDbContextCreator.Setup(m => m.Create(typeof(IIdentityInsertDbContext<User, int>)))
                                     .Returns(_MockUserDbContext.Object);
            var baseRepository = CreateBaseRepository();
        ...
        #region GetDistinctPropertyValues
        [TestMethod]
        public void BaseRepository_GetDistinctPropertyValues_Property_NotFound_Throws()
        {
            // Arrange
            string propertyName = "Test";
            Expression<Func<User, bool>> preExpression = null;
            ...
            {
                baseRepository.GetDistinctPropertyValues(propertyName, preExpression);
            });
            _MockRepository.VerifyAll();
        }
        #endregion
