...
namespace Rhyous.EntityAnywhere.Clients2
{
        ...
        where TE2Id : IComparable, IComparable<TE2Id>, IEquatable<TE2Id>
    {
        ...
        /// <summary>
        /// Gets the mapped entity by a list of Mappings, which are instances of TEntity but only the two mapping fields populated.
        /// </summary>
        /// <param name="mappings">Instances of TEntity but only the two mapping fields populated</param>
        /// <param name="forwardExceptions">Whether or not we want the exception forwarded on to be displayed as a service error response message (true) or to be treated as an empty result set (false).</param>
        /// <returns>A list of mapped entities.</returns>
        Task<OdataObjectCollection<TEntity, TId>> GetByMappingsAsync(IEnumerable<TEntity> mappings, bool forwardExceptions = true);
        ...
        /// <param name="forwardExceptions">Whether or not we want the exception forwarded on to be displayed as a service error response message (true) or to be treated as an empty result set (false).</param>
        /// <returns>A list of mapped entities.</returns>
        Task<OdataObjectCollection<TEntity, TId>> GetByMappingsAsync(IEnumerable<TEntity> mappings, string urlParameters, bool forwardExceptions = true);
    }
