...
namespace Rhyous.EntityAnywhere.Interfaces.Common.Tests
{
    ...
    public class PropertyOrderSorterTests
    {
        ...
        {
            return new PropertyOrderSorter(PreferentialPropertyComparer.Instance);
        }

        #region sort
        [TestMethod]
        public void PropertyOrderSorter_Sort_All0_Test()
        {
            // Arrange
            var unitUnderTest = CreatePropertyOrderSorter();
            var ep1 = new EntityProperty { Id = 1, Name = "Id", Order = 0, EntityId = 27 };
            var ep2 = new EntityProperty { Id = 2, Name = "Name", Order = 0, EntityId = 27 };
            var ep3 = new EntityProperty { Id = 3, Name = "Prop3", Order = 0, EntityId = 27 };
            var ep4 = new EntityProperty { Id = 4, Name = "Prop4", Order = 0, EntityId = 27 };
            var ep5 = new EntityProperty { Id = 5, Name = "Prop5", Order = 0, EntityId = 27 };
        ...
        public void PropertyOrderSorter_UpdateSortOrder_Test()
        {
            ...
            // Act
            var result = unitUnderTest.UpdateSortOrder(list, p => p.Name, PreferentialPropertyComparer.Instance)
                                      .ToList();
        ...
        #region Collate
        [TestMethod]
        public void PropertyOrderSorter_Collate_StateUnderTest_ExpectedBehavior()
        {
            // Arrange
        ...
        #region SafeInsert
        [TestMethod]
        public void PropertyOrderSorter_SafeInsert_0_GoesLast()
        {
            // Arrange
            var list = new List<EntityProperty>();
            list.Add(new EntityProperty());
            var secondEntityProperty = new EntityProperty();
            var propertyOrderSorter = CreatePropertyOrderSorter();

            // Act
            propertyOrderSorter.SafeInsert(list, secondEntityProperty);

            // Assert
            Assert.AreEqual(list[1], secondEntityProperty);
        }

        [TestMethod]
        public void PropertyOrderSorter_SafeInsert_OrderValueLargerThanListCount_GoesLast()
        {
            // Arrange
            var list = new List<EntityProperty>();
            list.Add(new EntityProperty());
            var secondEntityProperty = new EntityProperty { Order = int.MaxValue };
            var propertyOrderSorter = CreatePropertyOrderSorter();
