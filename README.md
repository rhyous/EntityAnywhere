...
2. Create an MS SQL database called EntityAnywhere on your local box.
   Note: The connection string is located under WebApi.Main\app.config and is called "SqlRepository"

3. Note your EntityAdminToken in the WebApi.Main\app.config.
   You can change this from the default if you would like. You can make it be any string you want.
   Default: ba258083519e2cb36d59b6105531cef2529a83e87826f3448e1f06e9b16ca400

3. Build and Run. 
   A swagger page should open.

You have many common Entities built-in.

4. Search the page for the RootWebService

5. Run these three endpoints in order to configure the database.

   - /api/Service/$Generate = This creates the tables.
   - /api/Service/$Seed = This seeds basic data in the tables.
   - /api/Service/$EntitySettings = This seeds basic entity data into the tables.

   To run these, follow these commands:

   1. Expand RootWebService by clicking on the arrow on the right.
   2. Click Try it out.
   3. Enter the EntityAdminToken value.
   4. Click Execute.

6. You can now log in with these credentials:
   User:        admin
   Password:    @dmin1234!
